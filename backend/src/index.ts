...
app.post("/api/chat/completions", aiChatRateLimiter, async (request, response) => {
  const body = parseObjectBody(request.body, response);
  if (!body) {
    return;
  }
  ...
  if (!apiKey) {
    response.status(500).json({ error: "OPENROUTER_API_KEY is missing. Configure it in the backend environment." });
    return;
  }

  const model = typeof body.model === "string" && body.model.trim().length > 0
    ? sanitizePlainText(body.model, 200)
  ...
  for (const item of rawMessages.slice(-40)) {
    ...
    const msg: Record<string, unknown> = {
      role,
      content: sanitizePlainText(content, 12_000)
    };

    // Pass-through any model-specific continuation metadata (ex: OpenRouter reasoning_details)
    if ("reasoning_details" in (item as Record<string, unknown>)) {
      msg.reasoning_details = (item as Record<string, unknown>).reasoning_details;
    }
    messages.push(msg);
  }

  if (messages.length === 0) {
    response.status(400).json({ error: "No valid messages provided." });
    return;
  }

  const reasoning = (body as { reasoning?: unknown }).reasoning;
  const includeReasoning = Boolean(
    reasoning &&
      typeof reasoning === "object" &&
      !Array.isArray(reasoning) &&
      (reasoning as { enabled?: unknown }).enabled === true
  );
  ...
  try {
    const upstream = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model,
        messages,
        ...(includeReasoning ? { reasoning: { enabled: true } } : {})
      })
    });
    ...
    if (!contentType.includes("application/json")) {
      response.status(502).json({ error: "OpenRouter returned non-JSON response.", details: text.slice(0, 2000) });
      return;
    }

    const payload = JSON.parse(text) as unknown;

    const maybeUserId = (request as Partial<AuthedRequest>).user?.id;
    if (typeof maybeUserId === "number") {
      await getDbPool().query(
        `
          ...
          VALUES($1, $2, $3::jsonb)
        `,
        [maybeUserId, "openrouter_chat_completions", JSON.stringify({ model, messageCount: messages.length })]
      );
    }

    response.json(payload);
  } catch (error) {
    logApiError(request, error, { route: "/api/chat/completions" });
    response.status(500).json({ error: error instanceof Error ? error.message : "OpenRouter request failed." });
  }
});
...
function sanitizeHistory(history: unknown): ChatMessage[] {
  ...
  for (const candidate of history) {
    ...
    if ((role === "user" || role === "assistant") && typeof content === "string" && content.trim().length > 0) {
      const message: ChatMessage = { role, content: sanitizePlainText(content, 4000) };
      if (role === "assistant" && "reasoning_details" in (candidate as Record<string, unknown>)) {
        message.reasoning_details = (candidate as Record<string, unknown>).reasoning_details;
      }
      clean.push(message);
    }
  }
